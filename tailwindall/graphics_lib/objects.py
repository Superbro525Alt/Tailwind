...
class Sprite(classes.BaseObject):
    def __init__(self, image, position, size, sprite=None, setup=False):
        self.image = image
...
class Rectangle(Sprite):
    def __init__(self, color, position, size, setup=False):
        self.color = color
        super().__init__(None, position, size, None, setup)
        self.setup = setup

    def _calculate_sprite(self, screen):
        super().__init__(None, self.position, self.size,
                         pygame.Rect(self.position[0], self.position[1], self.size[0], self.size[1]), setup=False)
...
class Polygon(Sprite):
    def __init__(self, shape: Union[list[tuple[int, int]], shapes.Shape], color, position, show_points=False,
                 points_color=None, points_radius=5, show_angles=False, show_side_lengths=False, show_name=False,
                 gap=50):
        ...
        if self.shape is not None:
            ...
            if self.show_angles:
                for i in range(len(self.shape.angles)):
                    text = pygame.font.Font("./fonts/FreeSansBold.ttf", 20).render(f"{str(int(self.shape.angles[i]))}°",
                                                                                   True, (0, 0, 0))
                    screen.blit(text, (self.shape.points[i][0], self.shape.points[i][1]))

            if self.show_side_lengths:
                for i in range(len(self.shape.sides)):
                    text = pygame.font.Font("./fonts/FreeSansBold.ttf", 20).render(f"{str(int(self.shape.sides[i]))}",
                                                                                   True, (0, 0, 0))
                    if not i == len(self.shape.sides) - 1:
                        screen.blit(text, ((self.shape.points[i][0] + self.shape.points[i + 1][0]) / 2,
                                           (self.shape.points[i][1] + self.shape.points[i + 1][1]) / 2))
                    else:
                        screen.blit(text, ((self.shape.points[i][0] + self.shape.points[0][0]) / 2,
                                           (self.shape.points[i][1] + self.shape.points[0][1]) / 2))

            if self.show_name:
                text = pygame.font.Font("./fonts/FreeSansBold.ttf", 20).render(f"{self.shape.name}", True, (0, 0, 0))
                center_of_shape = (sum([i[0] for i in self.shape.points]) / len(self.shape.points),
                                   sum([i[1] for i in self.shape.points]) / len(self.shape.points))
                center_of_shape = (
                center_of_shape[0] - text.get_width() / 2, center_of_shape[1] - text.get_height() / 2)
